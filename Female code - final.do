...
save "$output\\Final_fileb_female", replace

global control age i.country 

use "$output\\Final_fileb_female", clear
...
keep if e(sample)
save "$working\\Final_fileb_female_reduit", replace 
...
save first_reg_female.dta, replace  

///Question 1: does taking the main job or the entire career change something? -> FEMALE 
...
corr est1 est2
**95.07 % of correlation between raw coefficient
...
graph export "$working\\Picture_main_job_time_female.png", as(png) replace
...
graph export "$working\\Picture_main_first_last_job_female.png", as(png) replace

corr estimate_main  estimate_first estimate_last
*84.80 between main and firstt 
*96.73 between main and last 
...
use "$working\\Final_fileb_female_reduit" 
...
graph export "$working\\Picture_baseline_educ_educ_child_female.png", as(png) replace

corr est*
*96.00 between main and edu 
*93.85 between main and complete 
...
save "$output\\Final_filec_female", replace

forvalues i = 1/3 {

use "$output\\Final_filec_female", clear
...
save "$output\\Final_fileb70_female", replace 

use "$output\\Final_fileb70_female", clear
...
save "$output\\Final_filebr_female", replace


use "$output\\Final_filebr_female", clear
